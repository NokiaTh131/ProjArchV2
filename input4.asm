...
pnrAddr .fill pnr        ; Address of the Cnr function
...
main    lw 0 x2 stAddr      ; x2 = stack address
        lw 0 x12 n           ; x12 = n
        lw 0 x13 r           ; x13 = r
        lw 0 x5 pnrAddr      ; Load the address of Cnr into x5
        jalr x5 x1          ; Jump to Cnr function
        halt                ; End of progx1m
pnr     beq 0 x13 bax2nr     ; if r == 0, go to bax2nr (base case)
        lw 0 x5 pnrAddr      ; Load the address of Pnr into x5 (for recursion)
        lw 0 x6 pos3
...
mult    add 0 0 x5       ; Initialize x5 to 0 (this will hold the result)
        beq x12 0 doneMult  ; If x12 == 0, return (multiplication by 0 results in 0)
multLoop add x10 x5 x5      ; (x5 = x5 + x12)
        nand 0 0 x6       ; Decrement x12 (we use nand to negate, then add 1)
        add x12 x6 x12     ; x12 = x12 - 1 -> n-=1
        beq x12 0 doneMult  ; If x12 == 0, we're done
        beq 0 0 multLoop   ; Repeat the loop
doneMult add 0 x5 x10       ; Move the result from x5 to x10 (x10 = x5)
        jalr x1 0          ; Return to caller
